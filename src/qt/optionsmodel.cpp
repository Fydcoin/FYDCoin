...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::FYD);
    ...
    if (!settings.contains("nAnonymizeFydCoinAmount"))
        settings.setValue("nAnonymizeFydCoinAmount", 1000);

    nAnonymizeFydCoinAmount = settings.value("nAnonymizeFydCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeFydCoinAmount"))
        SoftSetArg("-anonymizefydamount", settings.value("nAnonymizeFydCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in fyd.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeFydAmount:
            return QVariant(nAnonymizeFydCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeFydAmount:
            nAnonymizeFydCoinAmount = value.toInt();
            settings.setValue("nAnonymizeFydCoinAmount", nAnonymizeFydCoinAmount);
            emit anonymizeFydAmountChanged(nAnonymizeFydCoinAmount);
            break;
        case CoinControlFeatures:
