...
#include "ui_zfydcontroldialog.h"
...
std::set<std::string> ZFydControlDialog::setSelectedMints;
std::set<CMintMeta> ZFydControlDialog::setMints;

bool CZFydControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZFydControlDialog::COLUMN_DENOMINATION || column == ZFydControlDialog::COLUMN_VERSION || column == ZFydControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZFydControlDialog::ZFydControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZFydControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZFydControlDialog::~ZFydControlDialog()
{
    delete ui;
}

void ZFydControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZFydControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZFydControlWidgetItem* itemDenom(new CZFydControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZFydControlWidgetItem *itemMint = new CZFydControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "The zFYD seed used to mint this zFYD is not the same as currently hold in the wallet";
            else
...
void ZFydControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZFydControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZFyd_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZFydControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZFydControlDialog::GetSelectedMints()
{
...
void ZFydControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
