...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZFydStake(spend));
    } else {
        ...
        CFydStake* fydInput = new CFydStake();
        fydInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(fydInput);
    }
