...
public:
    ...
    bool WriteZFYDSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZFYDSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZFYDSeed_deprecated(uint256& seed);
    bool EraseZFYDSeed();
    bool EraseZFYDSeed_deprecated();

    bool WriteZFYDCount(const uint32_t& nCount);
    bool ReadZFYDCount(uint32_t& nCount);
