...
CzFYDTracker::CzFYDTracker(std::string strWalletFile)
{
...
CzFYDTracker::~CzFYDTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzFYDTracker::Init()
{
...
bool CzFYDTracker::Archive(CMintMeta& meta)
{
...
bool CzFYDTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzFYDTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzFYDTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzFYDTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzFYDTracker::GetSerialHashes()
{
...
CAmount CzFYDTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzFYDTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzFYDTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzFYDTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzFYDTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzFYDTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzFYDTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzFYDTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzFYDTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzFYDTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzFYDTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzFYDWallet* zFYDWallet)
{
    bool iszFYDWalletInitialized = (NULL != zFYDWallet);
    CMintMeta meta;
    ...
    if (! iszFYDWalletInitialized)
        zFYDWallet = new CzFYDWallet(strWalletFile);
    meta.isSeedCorrect = zFYDWallet->CheckSeed(dMint);
    if (! iszFYDWalletInitialized)
        delete zFYDWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzFYDTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzFYDTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzFYDTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzFYDTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzFYDTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzFYDTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzFYDWallet* zFYDWallet = new CzFYDWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zFYDWallet);
        }
        delete zFYDWallet;
        LogPrint("zero", "%s: added %d dzfyd from DB\n", __func__, listDeterministicDB.size());
    }
