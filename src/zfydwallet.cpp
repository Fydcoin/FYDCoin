...
CzFYDWallet::CzFYDWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zfyd seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZFYDSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZFYDSeed_deprecated()) {
                    LogPrintf("%s: Updated zFYD seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zfyd seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zfyd wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzFYDWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZFYDCount(nCountLastUsed);
    else if (!walletdb.ReadZFYDCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzFYDWallet::Lock()
{
    seedMaster = 0;
}

void CzFYDWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzFYDWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZFYD(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzFYDWallet::LoadMintPoolFromDB()
{
...
void CzFYDWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzFYDWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzFYDWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zfydTracker->HasPubcoinHash(pMint.first)) {
...
bool CzFYDWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZFYD(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zfydTracker which also adds to database
    pwalletMain->zfydTracker->Add(dMint, true);
...
void CzFYDWallet::SeedToZFYD(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzFYDWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzFYDWallet::GenerateDeterministicZFYD(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzFYDWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzFYDWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzFYDWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
